...
inline const char *ftimestamp(int64_t t, char *buf) {
    if (PTS_NO_VALUE == t) {
        ...
        t /= 90;
        sprintf(buf, "%d:%02d:%02d.%03d", (int) (t / 3600000), (int) ((t / 60000) % 60), (int) ((t / 1000) % 60),
                (int) (t % 1000));
    }
    return buf;
}
...
/* 解析ps数据 */
static void
onpacket(void * /*param*/, int /*stream*/, int avtype, int flags, int64_t pts, int64_t dts, const void *data,
         size_t bytes) {
...
public:
    ps_muxer_context(char *input_file, int video, int audio);
...
ps_muxer_context::ps_muxer_context(char *input_file, int video, int audio) {
    memset(output, 0, 256);
    memset(input, 0, 256);
    ...
    if (audio != PSI_STREAM_RESERVED) {
        audio_id = ps_muxer_add_stream(ps, audio, NULL, 0);
    }
    if (video != PSI_STREAM_RESERVED) {
        video_id = ps_muxer_add_stream(ps, video, NULL, 0);
    }
...
//                    }
//
//                    ps_muxer_input(ps, video_id, flags, video.time_stamp, video.time_stamp, video.data,
//                                   video.data_size);
...
//            } else {
            ...
            if (audio.data && audio.data_size > 0) {
                ...
                ps_muxer_input(ps, audio_id, flags, audio.time_stamp, audio.time_stamp, audio.data,
                               audio.data_size);
...
//    ps_muxer_context ps_muxer(file,PSI_STREAM_H264,PSI_STREAM_AAC);

    char *file = "../ost_alaw.wav";
    ps_muxer_context ps_muxer(file, PSI_STREAM_RESERVED, PSI_STREAM_AUDIO_PCMA);
